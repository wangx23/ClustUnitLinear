...
#'@param x x without intercept, will add intercept
#'@export
                         ...
                         tol = 1e-3)
{
  ...
  z <- cbind(1, x[,-(index_d), drop = FALSE]) ## common part
  ...
  out <- list(eta = eta_new, beta = betam_new, tau0 = tau0_new, tau1 = tau1_new,
  ...
  # if(len_d == ncol(x))
  # {
  #   Xty <- t(wts_tilde*Xm) %*% y
  #   Xinv <- inverseR(dom, x_d*sqrt(wts_tilde), lam0)
  #   beta00_vec <- Xinv %*% Xty
  #   beta00 <-  matrix(beta00_vec,ncol= len_d, byrow = TRUE)
  #
  #
  ...
  #   vv_cur <- rep(0, nr) ### random effects
  #
  #  # tau00 <- mean((y - x_d %*%solve(t(x_d) %*% x_d) %*% t(x_d) %*%y)^2)
  #
  ...
  #
  #     ## update vv ##
  #     Vinv <- 1/(as.numeric(by(as.numeric(wm), dom, sum)) + 1/tau1_cur)
  #     vv_new <- Vinv * t(wm * Hmat) %*% (y - Xm %*% beta_new)
  #
  #
  #     #### update tau0 and tau1 ##
  #
  #     tau0_new <- mean((y - Xm %*% beta_new - vv_new[dom])^2)
  #     tau1_new <- mean(Vinv + vv_new^2)
  #
  #     ###
  #     psim <- D %*% betam_new + um/nu
                                 ...
                                 tol = 1e-3)
{
  ...
  res_fitted <- Clust_nested_info(dom, y, x, wts, index_d,
                                 lam = lamvec[which.min(bic_df$mbic)],
                                 nu = nu, gam = gam, lam0 = lam0,
                                 maxiter = maxiter, tol = tol)
